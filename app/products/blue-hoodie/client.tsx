...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    const found = cart.find((item: any) => item.id === 'blue-hoodie' && item.size === size);
  ...
  const handleAddToCart = () => {
    ...
    const cart = JSON.parse(localStorage.getItem('cart') || '[]');
    const exists = cart.some((item: any) => item.id === 'blue-hoodie' && item.size === size);
    if (exists) {
    ...
    <div className="min-h-screen flex flex-col md:flex-row bg-white">
      <div className="md:w-1/2 w-full p-6 flex flex-col items-center justify-center">
        ...
        <div className="flex gap-3 mt-4 flex-wrap justify-center">
          {images.map((img, i) => (
            <div
              key={i}
              className={`relative w-20 h-20 border rounded cursor-pointer ${selectedImage === img ? 'ring-2 ring-blue-500' : ''}`}
      ...
      <div className="md:w-1/2 w-full p-6 md:p-12 flex flex-col justify-center space-y-5">
        ...
        <div className="text-gray-700 text-sm leading-relaxed">
          <p><strong>Description:</strong> A premium urban hoodie crafted from organic cotton.</p>
